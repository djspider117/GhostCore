...
{
    /// <summary>
    /// A three-dimensional gradient noise field
    /// </summary>
    /// <remarks>
    ...
    public class GradientNoiseField : Field
    {
        ...
        /// <summary>
        /// The SCurved used to interpolate values in the noise generation algorithm
        /// </summary>
        public SCurve SCurve { get { return _scurve ?? SCurve.Default; } }

        /// <summary>
        /// Create new generator with specified seed
        /// </summary>
        /// <param name="seed">noise seed</param>
        public GradientNoiseField(int seed)
            : this(seed, null)
        {
        }
        /// <summary>
        ...
        public GradientNoiseField(int seed, SCurve sCurve)
        {
            _seed = seed;
            _scurve = sCurve;
        }
        ...
        public override float GetValue(float x, float y, float z)
        {
            ...
            var xs = SCurve.GetValue(x - ix);
            var ys = SCurve.GetValue(y - iy);
            var zs = SCurve.GetValue(z - iz);
            ...
            var ix0 = MathUtils.Lerp(xs, n0, n1);

            n0 = GetNoise(x, y, z, ix, iy + 1, iz);
            n1 = GetNoise(x, y, z, ix + 1, iy + 1, iz);
            var ix1 = MathUtils.Lerp(xs, n0, n1);

            var iy0 = MathUtils.Lerp(ys, ix0, ix1);

            n0 = GetNoise(x, y, z, ix, iy, iz + 1);
            n1 = GetNoise(x, y, z, ix + 1, iy, iz + 1);
            ix0 = MathUtils.Lerp(xs, n0, n1); // on y=0, z=1 edge

            n0 = GetNoise(x, y, z, ix, iy + 1, iz + 1);
            n1 = GetNoise(x, y, z, ix + 1, iy + 1, iz + 1);
            ix1 = MathUtils.Lerp(xs, n0, n1); // on y=z=1 edge

            var iy1 = MathUtils.Lerp(ys, ix0, ix1);

            return MathUtils.Lerp(zs, iy0, iy1); // inside cube		}
        }
        ...
        private Vector3 GetRandomVector(int x, int y, int z)
        {
            ...
            int vectorIndex = (
               ...
               + Constants.MultiplierSeed * _seed)
               & 0x7fffffff; // <- don't ask
    ...
    {
        public const int MultiplierX = 9584;
        public const int MultiplierY = 32842;
        public const int MultiplierZ = 41132;
        public const int MultiplierSeed = 654;
